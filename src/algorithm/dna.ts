...
class DNA {
    ...
    calcFitness() {
        ...
        const overStuffing =  this.calculateOverStuffing(this.genes)
        // console.log("this is overstuffing in each loop", overStuffing);
        
        let score: number = 0;
        if(overStuffing == 0){
            score = 100
        }else if(overStuffing>5){
            score = -50
        } else{
            score = -20
        }
    ...
    calculateOverStuffing(dna: any[]) {
        ...
        crossover(partner: DNA) {
            ...
            let child = new DNA();
            // console.log(child);
            let midpoint = Math.floor(Math.random() * TARGET.length); // Pick a midpoint
